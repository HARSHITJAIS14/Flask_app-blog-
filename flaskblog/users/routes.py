...
@login_required   #so the user will only go to this page if it is login... else will go to login page which we have set in init with login_view
def account():
...
def reset_request():
    ...
    if form.validate_on_submit():
         ...
         flash('An email has been sent with further instruction to reset password','success')
         return redirect(url_for('users.login'))
